...
module.exports = {
  async up(queryInterface, Sequelize) {
    console.log("Iniciando Seeder de horarios_citas_licencias desde CSV...");

    // Ruta del CSV (ajusta según donde esté tu archivo)
    const csvFilePath = path.resolve(__dirname, '../database/catalogos/horarioLicencias.csv');

    // Vaciar tabla antes de insertar
    await queryInterface.bulkDelete('horarios_licencias', null, {});
      ...
      fs.createReadStream(csvFilePath)
        ...
        .on('end', async () => {
          ...
          if (horarios.length > 0) {
            await queryInterface.bulkInsert('horarios_licencias', horarios, {});
          }

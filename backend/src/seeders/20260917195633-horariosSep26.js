...
module.exports = {
  async up(queryInterface, Sequelize) {
    console.log("Iniciando Seeder de horarios_citas_sep desde CSV...");

    // Ruta del CSV (ajusta según donde esté tu archivo)
    const csvFilePath = path.resolve(__dirname, '../database/catalogos/horarioSep26.csv');

    // Vaciar tabla antes de insertar
    await queryInterface.bulkDelete('horarios_citas_sep', null, {});
      ...
      fs.createReadStream(csvFilePath)
        ...
        .on('end', async () => {
          ...
          if (horarios.length > 0) {
            await queryInterface.bulkInsert('horarios_citas_sep', horarios, {});
          }

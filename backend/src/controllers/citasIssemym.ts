...
import HorarioIssemym from "../models/horarios_issemym"; // ✅ corregido
...
export const getHorariosDisponibles = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const horariosDisponibles = await HorarioIssemym.findAll({
      order: [["id", "ASC"]],
    });
...
export const savecita = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const cantidadCitas = await citasIssemym.count({
      where: {
    ...
    const cita = await citasIssemym.create({
      horario_id: body.horario_id,
      sede_id: body.sede_id,
      rfc: body.rfc,
      fecha_cita: body.fecha_cita,
...
export const generarPDFCitas = async (req: Request, res: Response) => {
  try {
    ...
    const citas = await citasIssemym.findAll({
      ...
      order: [["horario_id", "ASC"]],
      raw: false
    }) as (citasIssemym & { Sede?: { sede: string }, usuario?: any })[];
...
export const generarPdfAcuse = async (req: Request, res: Response) => {
  try {
    ...
    const citaHora =  cita?.HorarioIssemym?.horario_inicio + '-' + cita?.HorarioIssemym?.horario_fin;
    const pdfBuffer = await generarPDFBuffer({
      folio: cita.folio,
      nombreCompleto: nombreCompleto,
      sexo: '',
...
export const generalExcel = async (req: Request, res: Response) => {
  try {
    const citas = await citasIssemym.findAll({
      ...
      order: [["horario_id", "ASC"]],
      raw: false,
    }) as (citasIssemym & { Sede?: { sede: string }; datos_user?: any; dependencia?: any })[];
...
export const saveCitaSep = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const cantidadCitas = await CitaSep.count({
     ...
     const cantidadCitasDia = await CitaSep.count({
      where: {
        fecha_cita: body.fecha_cita
      }
    });
...
export const generarPdfAcuseSep = async (req: Request, res: Response) => {
  try {
    ...
    const tramites = cita.tramites.split(',').map((tramite: string): string | undefined => {
      const id = Number(tramite.trim());
        if (id === 1) {
          return 'Credencialización ';
        }

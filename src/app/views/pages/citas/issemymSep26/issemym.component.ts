...
@Component({
  ...
  templateUrl: './issemym.component.html',
  styleUrl: './issemym.component.scss'
})
export class IssemymComponent {
  ...
  public _citasService = inject(CitasIssemymService);
  ...
  calendarOptions: CalendarOptions = {
    ...
    dateClick: (info) => {
      ...
      if (this.highlightedDates.includes(clickedDate)) {
        ...
        this._citasService.getCitasSep(clickedDate).subscribe({
          next: (response: any) => {
            const sedeFija = this.fechaCitaEnvio == '2026-09-24' ? 3 : (this.fechaCitaEnvio == '2026-09-25' ? 3 : null);
  ...
  guardarSeleccion() {
    ...
    this._citasService.saveCitaSep(datos).subscribe({
      next: (response: any) => {
  ...
  abrirModal(persona: any) {
    ...
    if (this.fechaCitaEnvio === '2026-02-24') {
  ...
  descargarpdf() {
    this._citasService.generarPdfinalSep(this.currentUser.rfc).subscribe({
      next: (res: Blob) => {
